...
function BlockFrameImpl({ blockId }: { blockId: string }) {
  ...
  return (
    <div ref={setNodeRef} data-block-id={blockId} style={wrapperStyle} className="group relative">
      <div
        role="presentation"
      ...
      <div
        ...
        className={`absolute -left-7 top-1 flex flex-col gap-1 transition
          ${selected ? "opacity-100" : "opacity-0 group-hover:opacity-100"}`}
      >
        <button
          type="button"
          aria-label="Move block"
          className="flex h-6 w-6 cursor-grab items-center justify-center rounded-md border border-line
                     bg-white text-ink-400 shadow-sm hover:text-brand-600 active:cursor-grabbing"
          {...attributes}
          {...listeners}
        >
        ...
        <button
          ...
          className="flex h-6 w-6 items-center justify-center rounded-md border border-line bg-white
                     text-ink-400 shadow-sm hover:border-red-200 hover:text-red-600"
